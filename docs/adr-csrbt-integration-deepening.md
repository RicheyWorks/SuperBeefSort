...
**Decision (smallest, last):** allow `buildCoOptimized` to derive the `ProfileGuidedScorer` prior strength from realized run signals (e.g. a stronger prior when the sort was cheap/clean, weaker when the profile was uncertain). Low priority; design-only until Gaps 1–3 land.
...
2. [ ] **Gap 1:** `csrbt/EnsembleAdaptation<K>` + `BeefSort.buildAdaptiveEnsemble(MorphPolicy)`; integration test exercising `evaluateAndMaybePromote` and a forced member failover (quarantine → heal).
3. [ ] **Gap 2:** `EnsembleSpec` + `EnsembleTargetFactory.forProfile(profile, access, spec)` + `BeefSort.buildEnsemble(EnsembleSpec)`; default spec == current mix; add a `persistentMember()` when `spec.snapshot()`.
4. [ ] **Gap 4:** optional `OrderStats<K>` view (or tested recipes) delegating to CSRBT order-statistics; end-to-end test sort → build → median/percentile/rangeQuery.
5. [ ] **Gap 5 (design-only):** sketch deriving the `ProfileGuidedScorer` prior strength from realized `SortRunResult` signals; defer implementation until 1–3 land.
